...
namespace Engine.Network
{
    public class Client
    {
        ...
        public void Update()
        {
            if(!session.clients.Contains(this))
                session.TryAddClient(this);
            else
            {
                ServerSend.ReturnClientsUDP(id);
            }
        }
            ...
            while (_packetLength > 0 && _packetLength <= TcpReceivedData.UnreadLength())
            {
                ...
                ThreadManagerServer.ExecuteOnMainThread(() =>
                {
                    using (Packet _packet = new Packet(_packetBytes))
                    {
                        ...
                        Server.packetHandlers[_packetId](id, _packet,session);
        ...
        public class UDP
        {
            ...
            public void HandleData(Packet _packetData)
            {
                ...
                ThreadManager.ExecuteOnMainThread(() =>
                {
                    ...
                    {
                        int _packetId = _packet.ReadInt();
                        Server.packetHandlers[_packetId](id, _packet, session); // Call appropriate method to handle the packet
                    }
                });
            }
        }
